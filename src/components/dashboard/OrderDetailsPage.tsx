...
import { ArrowRight, Package, Truck, CheckCircle, Clock, MapPin, Phone, Mail, User, CreditCard, FileText, Loader2 } from "lucide-react";
...
const OrderDetailsPage = ({ orderId, onBack }: OrderDetailsProps) => {
  ...
  const statusColor = (status: string) => {
    switch (status) {
      case "مكتمل": return "bg-primary/10 text-primary";
      case "قيد التنفيذ": return "bg-accent/10 text-accent";
      case "جديد": return "bg-secondary text-foreground";
        ...
        <div className="lg:col-span-2 space-y-6">
                ...
                <tbody>
                  {order.items?.map((item) => (
                    <tr key={item.id} className="border-b border-border/50">
                      ...
                      <td className="px-5 py-3 font-medium text-foreground">{(parseFloat(item.price.replace(/[^\d.]/g, '')) * item.quantity).toLocaleString()} ر.س</td>
                    </tr>
                  ))}
                </tbody>
              </table>
          ...
          <div className="bg-card rounded-xl border border-border/50 p-5">
            ...
            <div className="flex items-center gap-4">
               <div className={`w-12 h-12 rounded-full flex items-center justify-center shrink-0 ${order.status === 'مكتمل' ? "bg-primary text-primary-foreground" : "bg-accent/10 text-accent"}`}>
                  {order.status === 'مكتمل' ? <CheckCircle size={24} /> : <Clock size={24} />}
               </div>
               <div>
                 <p className="font-bold text-foreground">{order.status}</p>
          ...
          <div className="bg-card rounded-xl border border-border/50 p-5">
            ...
            <div className="space-y-3">
              ...
              <div className="flex items-center gap-3 text-sm">
                ...
                <span className="text-foreground" dir="ltr">قيد التنفيذ (معلومات غير متوفرة في ملخص الطلب)</span>
              </div>
            </div>
          </div>
          ...
          <div className="bg-card rounded-xl border border-border/50 p-5">
             ...
             <div className="space-y-3">
               ...
               <div className="flex justify-between text-sm">
                 ...
                 <span className="text-primary font-medium">مدفوع (افتراضي)</span>
               </div>
             </div>
          </div>
        </div>
